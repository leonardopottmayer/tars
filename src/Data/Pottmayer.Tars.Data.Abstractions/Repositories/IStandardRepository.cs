...
/// <summary>
/// Provider-agnostic CRUD/query repository over <typeparamref name="TEntity"/> keyed by <typeparamref name="TKey"/>,
/// covering get/add/update/remove, existence/count and dynamic queries.
/// <para>
/// This contract is expressed entirely in BCL types (<see cref="Expression{TDelegate}"/>, <see cref="QueryParams"/>)
/// so it is implemented identically by every data axis (relational, document, …). Domain repository interfaces
/// (e.g. <c>IUserRepository</c>) should extend this neutral contract — never a provider-specific one — so the
/// backend can be swapped without touching the domain. Providers that need extra surface expose it via their own
/// sub-interface (e.g. <c>IRelationalRepository</c> adds <c>Queryable()</c>).
/// </para>
/// </summary>
